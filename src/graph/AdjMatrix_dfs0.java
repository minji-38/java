...
//			탐색할 노드를 스택에서 추출
			...
			if (!visited[current]) { //방문배열은 노드개수와 동일한 길이
//			방문처리하고(스택은 추출 후 방문처리를 해줘야함 => 방문처리를 한것만 뽑아내야함)
				visited[current] = true;
//				추출한 노드에 대한 처리 
				System.out.printf(" %c", current + 65);

//				현재 노드의 인접된 노드에 대한 방문 시도
				for (int adj = N - 1; adj >= 0; adj--) { //stack으로 하면 0부터 넣으면오른쪽 노드부터 탐색하게됨 (거꾸로 마지막부터 넣고 --하는 형식으로~)
//					방문한 적이 없고 		인접 되어 있다면
